...
Matrix3x3 Matrix3x3::getR(const float aAngleInDegree) noexcept {
	float lAngleInRadians = aAngleInDegree * Pi / 180.0f;
	return Matrix3x3(Vector3D(std::cos(lAngleInRadians), -std::sin(lAngleInRadians), 0.0f),
		Vector3D(std::sin(lAngleInRadians), std::cos(lAngleInRadians), 0.0f),
		Vector3D(0.0f, 0.0f, 1.0f));
}
